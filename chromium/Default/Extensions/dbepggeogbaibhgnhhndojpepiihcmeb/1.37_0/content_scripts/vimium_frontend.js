...
(function() {
  var HUD, Tween, activatedElement, addCssToPage, bubbleEvent, checkIfEnabledForUrl, currentCompletionKeys, disableVimium, enterInsertModeIfElementIsFocused, enterInsertModeWithoutShowingIndicator, executeFind, exitFindMode, exitInsertMode, findAndFocus, findAndFollowLink, findAndFollowRel, findMode, findModeAnchorNode, findModeQuery, findModeQueryHasResults, focusFoundLink, followLink, frameId, getCurrentUrlHandlers, getLinkFromSelection, getNextQueryFromRegexMatches, handleDeleteForFindMode, handleEnterForFindMode, handleEscapeForFindMode, handleKeyCharForFindMode, handlerStack, hasModifiersRegex, hideHelpDialog, initializeOnDomReady, initializePreDomReady, initializeWhenEnabled, insertModeLock, isDOMDescendant, isEditable, isEmbed, isEnabledForUrl, isFocusable, isInsertMode, isShowingHelpDialog, isValidFirstKey, keyPort, linkHintCss, onBlurCapturePhase, onDOMActivate, onFocusCapturePhase, onKeydown, onKeypress, onKeyup, performFindInPlace, refreshCompletionKeys, registerFrameIfSizeAvailable, restoreDefaultSelectionHighlight, root, scrollActivatedElementBy, selectFoundInputElement, settings, showFindModeHUDForQuery, suppressEvent, textInputXPath, updateFindModeQuery, validFirstKeys;

  getCurrentUrlHandlers = [];
  ...
  settings = {
    ...
    values: {},
    loadedValues: 0,
    valuesToLoad: ["scrollStepSize", "linkHintCharacters", "filterLinkHints", "hideHud", "previousPatterns", "nextPatterns", "findModeRawQuery"],
    isLoaded: false,
    eventListeners: {},
    init: function() {
  ...
  initializePreDomReady = function() {
    ...
    chrome.extension.sendRequest({
      ...
      return linkHintCss = response.linkHintCss;
    });
    ...
    return chrome.extension.onConnect.addListener(function(port, name) {
      if (port.name === "executePageCommand") {
        return port.onMessage.addListener(function(args) {
          var i, _i, _ref;
          if (frameId === args.frameId) {
            if (args.passCountToFunction) {
              Utils.invokeCommandString(args.command, [args.count]);
            } else {
              for (i = _i = 0, _ref = args.count; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
                Utils.invokeCommandString(args.command);
              }
            }
          }
          return refreshCompletionKeys(args);
        });
      } else if (port.name === "getScrollPosition") {
        return port.onMessage.addListener(function(args) {
          var scrollPort;
          scrollPort = chrome.extension.connect({
            name: "returnScrollPosition"
          });
          return scrollPort.postMessage({
        ...
        return port.onMessage.addListener(function(args) {
          if (args.scrollX > 0 || args.scrollY > 0) {
            return DomUtils.documentReady(function() {
              return window.scrollBy(args.scrollX, args.scrollY);
            });
          }
        });
        ...
        return port.onMessage.addListener(function(args) {
          if (getCurrentUrlHandlers.length > 0) {
            return getCurrentUrlHandlers.pop()(args.url);
          }
        });
      } else if (port.name === "refreshCompletionKeys") {
        return port.onMessage.addListener(function(args) {
          return refreshCompletionKeys(args.completionKeys);
        });
  ...
  extend(window, {
    ...
    toggleViewSource: function() {
      var getCurrentUrlPort, toggleViewSourceCallback;
      toggleViewSourceCallback = function(url) {
      ...
      getCurrentUrlPort = chrome.extension.connect({
        name: "getCurrentTabUrl"
      });
      return getCurrentUrlPort.postMessage({});
    },
    copyCurrentUrl: function() {
      var getCurrentUrlPort;
    ...
    focusInput: function(count) {
      var currentInputBox, i, lastInputBox, results;
  ...
  bubbleEvent = function(type, event) {
    var i, _i, _ref;
    for (i = _i = _ref = handlerStack.length - 1; _ref <= 0 ? _i <= 0 : _i >= 0; i = _ref <= 0 ? ++_i : --_i) {
  ...
  selectFoundInputElement = function() {
    if (findModeQueryHasResults && DomUtils.isSelectable(document.activeElement) && isDOMDescendant(findModeAnchorNode, document.activeElement)) {
  ...
  findAndFocus = function(backwards) {
    ...
    elementCanTakeInput = DomUtils.isSelectable(document.activeElement) && isDOMDescendant(findModeAnchorNode, document.activeElement);
  ...
  findAndFollowLink = function(linkStrings) {
    var boundingClientRect, candidateLink, candidateLinks, computedStyle, exactWordRegex, i, link, linkMatches, linkString, links, linksXPath, wordCount, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _m, _n, _ref;
    ...
    candidateLinks = [];
    for (i = _i = _ref = links.snapshotLength - 1; _ref <= 0 ? _i <= 0 : _i >= 0; i = _ref <= 0 ? ++_i : --_i) {
    ...
    wordCount = function(link) {
      return link.innerText.trim().split(/\s+/).length;
    };
    ...
    candidateLinks = candidateLinks.sort(function(a, b) {
      ...
      return wordCount(a) <= wordCount(candidateLinks[0]) + 1;
    });
    for (_k = 0, _len1 = linkStrings.length; _k < _len1; _k++) {
      linkString = linkStrings[_k];
    ...
    for (_m = 0, _len3 = linkStrings.length; _m < _len3; _m++) {
      linkString = linkStrings[_m];
      for (_n = 0, _len4 = candidateLinks.length; _n < _len4; _n++) {
        candidateLink = candidateLinks[_n];
  ...
  window.showHelpDialog = function(html, fid) {
    ...
    container.getElementsByClassName("optionsPage")[0].addEventListener("click", function() {
      return chrome.extension.sendRequest({
